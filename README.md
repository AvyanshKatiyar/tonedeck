...
**Launch it like an app.** The installer drops a double-clickable **`ToneDeck.app`** on your Desktop — carrying a custom EQ-faders icon — a real single-instance macOS app (a native Cocoa + WKWebView host, not a browser shortcut) that makes sure the daemon is up and shows the UI in its own window. It behaves like any Mac app: clicking the icon when it's already running just brings the existing window to the front — one window, no duplicates. Drag it to the Dock or Applications if you like. Skip it at install time with `./scripts/install.sh --no-launcher`, or (re)build it anytime with `./scripts/make-launcher.sh`. The same EQ icon is the UI's favicon, and the top bar shows a **live / standby / offline** status pill so you can tell at a glance whether the daemon and CamillaDSP engine are running.
