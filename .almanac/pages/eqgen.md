...
sources:
  ...
  - id: session-sdk-conservative-on-god
    type: session
    session_id: f2217a2a-6265-4b07-9601-02820b669cfb
    note: Session on 2026-06-16T17:04Z (branch feat/eq-clustering-corpus, sdk-cli entrypoint) for "On God" by Kanye West (JESUS IS KING) using the conservative prompt. Agent returned a 6-band JSON directly in ~5 s (preamp -1, intent "warmth and clarity for sparse gospel production") with no tool calls and no skill activation, despite the tonedeck-eq skill appearing in the session's skill listing. Fourth confirmatory example of Pattern 2 (direct JSON, no tool calls). Cache token breakdown: 3 non-cached input tokens, 16945 cache-read, 14160 new ephemeral-1h cache created — consistent with a cold skill-listing cache being primed for the session.
  ...
  - id: session-sdk-conservative-skill-no-cli
    ...
    note: Session on 2026-06-16T17:05Z (branch feat/eq-clustering-corpus, sdk-cli entrypoint) for "Hands On (feat. Fred Hammond)" by Kanye West (JESUS IS KING) using the conservative prompt. Agent loaded the tonedeck-eq skill and read band-guide.md (two Read tool calls), then returned a 5-band JSON directly without executing any tonedeck CLI commands — despite Bash being available in SDK mode. Middle-ground behavioral pattern sub-variant A (no CLI at all): skill activates and reads domain knowledge, then returns JSON directly. Confirms the band-guide-then-direct-JSON pattern previously only observed in claude -p sessions also occurs non-deterministically in SDK sessions with the conservative prompt.
  - id: session-sdk-conservative-selah
    type: session
    session_id: eee2bccc-7b6a-44bf-a4bf-10d7aecd6aca
    note: Session on 2026-06-16T17:03Z (branch feat/eq-clustering-corpus, sdk-cli entrypoint) for "Selah" by Kanye West (JESUS IS KING) using the conservative prompt. Agent loaded the tonedeck-eq skill, ran tonedeck status --json (active preset was foster-the-people-pumped-up-kicks, dspState Running, dspVersion 4.1.3), read band-guide.md in parallel, then returned a 5-band JSON directly (preamp -1, cathedral weight / choir smoothed) with no further CLI calls — no tonedeck list, tonedeck show, or tonedeck create. Middle-ground behavioral pattern sub-variant B (status-then-direct-JSON): step 1 of the skill workflow ran, but the rest of the CLI sequence was suppressed. Distinguishes from b53964a7 where zero CLI commands ran.
  ...
  - id: session-sdk-conservative-use-this-gospel-direct
    type: session
    session_id: eb58bab5-6ca6-45d1-a6cc-8c2135a0ba43
    note: Session on 2026-06-16T17:09Z (branch feat/eq-clustering-corpus, sdk-cli entrypoint) for "Use This Gospel (feat. Clipse & Kenny G)" by Kanye West (JESUS IS KING) using the same conservative prompt. Agent returned JSON directly in ~7 s with no tool calls; response was code-fenced (```json...```) but handled by extractJson. This is the strongest single proof of SDK non-determinism in the corpus: the same song, same prompt, same branch, just 4 minutes after d9986553 triggered the full 8-step skill workflow, produced the opposite behavior — direct JSON with zero tool use.
  ...
  - id: session-sdk-immediate-500
    type: session
    session_id: e7030b44-662b-475d-b9ad-5934d676455f
    note: Session on 2026-06-16T17:27Z (branch feat/eq-clustering-corpus, sdk-cli entrypoint) for "See Me Now (feat. Beyoncé, Charlie Wilson & Big Sean) [Bonus Track]" by Kanye West (MBDTF) using the decisive prompt. API returned 500 immediately — input_tokens=0, output_tokens=0, model="<synthetic>" — before any model processing. Concurrent with the same ~17:25–17:27Z outage window. Distinguishes pre-model API-level 500 from the in-session 500 that terminated a5ccf506 after partial processing.
status: active
verified: 2026-06-17
---
...
2. **No tool calls**: Agent returns JSON directly without invoking any tools. Observed in sessions `7424d6cc` (~5.5 s, "Devil In a New Dress", conservative prompt), `9a1b3308` (~7 s, "All of the Lights (Interlude)", conservative prompt), `eb58bab5` (~7 s, "Use This Gospel", conservative prompt), and `f2217a2a` (~5 s, "On God", conservative prompt). [@session-sdk-direct-json] [@session-sdk-direct-json-interlude] [@session-sdk-conservative-use-this-gospel-direct] [@session-sdk-conservative-on-god] Session `eb58bab5` is the most direct evidence of SDK non-determinism in the corpus: `d9986553` ran the full 8-step workflow for the same song, same prompt, and same branch just 4 minutes earlier.

3. **Middle-ground — skill+band-guide, partial or no CLI**: Agent loads the `tonedeck-eq` skill and reads `band-guide.md`, but does not complete the full CLI workflow, then returns JSON directly. Two sub-variants have been observed:
   - **Sub-variant A — no CLI at all**: No `tonedeck` commands run. Observed in session `b53964a7` ("Hands On", conservative prompt). [@session-sdk-conservative-skill-no-cli] This pattern previously appeared only in `claude -p` sessions (where Bash is mechanically unavailable), but `b53964a7` confirms it also occurs in SDK mode where Bash IS available but the agent chose not to use it.
   - **Sub-variant B — status-then-direct-JSON**: `tonedeck status --json` runs (skill step 1), but `tonedeck list`, `tonedeck show`, and `tonedeck create` are never called. The agent reads `band-guide.md` in parallel with the status call, then returns JSON immediately after. Observed in session `eee2bccc` ("Selah", conservative prompt). [@session-sdk-conservative-selah] The distinction from sub-variant A matters: the agent committed to the skill workflow far enough to query live daemon state, then stopped — implying the suppression happens between step 1 and step 2, not before step 1.

The decisive vs. conservative prompt distinction is a weak signal, not a reliable predictor. `62fb0069` and `a5ccf506` (both decisive prompt) ran the full workflow; `7424d6cc`, `9a1b3308`, and `eb58bab5` (all conservative prompt) returned JSON with no tool calls. But `8bbf99d5` and `d9986553` (both conservative prompt, same branch) ran the full skill workflow, `b53964a7` (conservative prompt) hit middle-ground sub-variant A, and `eee2bccc` (conservative prompt) hit middle-ground sub-variant B. All three primary outcomes (and now two sub-variants of the middle-ground) have appeared with the conservative prompt in SDK mode. The sharpest evidence is that `d9986553` and `eb58bab5` used the same song ("Use This Gospel"), same conservative prompt, and same branch — and produced opposite behavioral patterns 4 minutes apart. Cache state, model sampling variance, and session context (skill listing, MCP server load sequence) are all plausible contributors. The JSON-only instruction cannot be relied on as a suppressor. The corpus build pipeline correctly uses `claude -p`; any refactoring that shifts generation to the SDK would need an explicit `--allowedTools []` constraint or equivalent to guarantee batch semantics across all runs.

**SDK sessions have additional failure modes beyond the main model.** The auto-mode safety classifier for Bash is a separate service path from the main model — it can become unavailable independently. When the classifier fails, Bash is fully blocked with the error "claude-sonnet-4-6 is temporarily unavailable, so auto mode cannot determine the safety of Bash right now." Four sessions during the same outage window (~17:25–17:27Z on 2026-06-16) illustrate four distinct failure and recovery patterns:

- **Read-tool fallback (session `a5ccf506`, "Lost In the World")**: After Bash was blocked, the agent continued and read `band-guide.md` via the Read tool. The session then terminated with a 500 Internal Server Error; no JSON was produced. The 500 here is an *in-session* termination — model processing started, the skill and Read tools ran, then the transport layer failed. [@session-sdk-decisive-bash-failure]
...
- **Immediate pre-model 500 (session `e7030b44`, "See Me Now")**: The API returned a 500 before any model processing — `input_tokens: 0`, `output_tokens: 0`, `model: "<synthetic>"`. No skill activation, no tool calls. This is distinct from `a5ccf506`'s in-session 500: the request never reached the model at all. Concurrent with the same outage window. [@session-sdk-immediate-500]

Read-only tools (Read, Grep, Glob) remain available at the infrastructure level when the classifier fails, but agents do not reliably use them. Whether an agent falls back to Read, retries Bash until termination, or retries until the classifier recovers appears to be a combination of model sampling variance and outage duration. The 500 Internal Server Error has two distinct manifestations: (a) a pre-model API rejection (0 tokens, synthetic response) where the request never reached the model, and (b) an in-session transport failure where the model had already begun processing. Both are terminal for that SDK session turn. These failures are transient and recoverable from the caller's perspective, but they are invisible to `eqgen.ts`'s error handling, which only catches process-level failures from `claude -p`, not SDK-level transport errors.
