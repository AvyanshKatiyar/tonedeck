...
**SDK sessions behave differently and non-deterministically.** When the same eqgen-style prompt is delivered via an SDK (`sdk-cli`) entrypoint rather than `claude -p`, the agent has full tool access. In some cases the skill activates and the agent executes the full 8-step workflow — `tonedeck status --json`, `tonedeck list --json`, `tonedeck show`, etc. — before producing output, as observed in sessions `62fb0069` (decisive prompt) [@session-sdk-eqgen-prompt], `a5ccf506` (decisive prompt, "Lost In the World") [@session-sdk-decisive-bash-failure], and `8bbf99d5` (conservative prompt). [@session-sdk-conservative-full-workflow] In other cases the agent returns JSON directly without invoking any tools, as observed in sessions `7424d6cc` (~5.5 s, "Devil In a New Dress") and `9a1b3308` (~7 s, "All of the Lights (Interlude)") — both conservative prompt, same branch, same day. [@session-sdk-direct-json] [@session-sdk-direct-json-interlude]

The decisive vs. conservative prompt distinction is a weak signal, not a reliable predictor. `62fb0069` and `a5ccf506` (both decisive prompt) ran the full workflow; `7424d6cc` and `9a1b3308` (both conservative prompt) returned JSON directly. However, session `8bbf99d5` used the identical conservative prompt on the same branch and same day and still ran the full skill workflow — loading `tonedeck-eq`, running `tonedeck status --json`, scanning the preset list with `tonedeck list --json`, reading `band-guide.md`, and ultimately creating `track-jesus-is-lord` via `tonedeck create --from-json --apply` rather than returning raw JSON. [@session-sdk-conservative-full-workflow] The conservative prompt therefore provides no guarantee across sessions. Cache state, model sampling variance, and session context (skill listing, MCP server load sequence) are all plausible contributors. The JSON-only instruction cannot be relied on as a suppressor. The corpus build pipeline correctly uses `claude -p`; any refactoring that shifts generation to the SDK would need an explicit `--allowedTools []` constraint or equivalent to guarantee batch semantics across all runs.

**SDK sessions have additional failure modes beyond the main model.** In session `a5ccf506`, the agent's Bash call for `tonedeck status --json` was blocked by the error "claude-sonnet-4-6 is temporarily unavailable, so auto mode cannot determine the safety of Bash right now." The auto-mode safety classifier for Bash is a separate service path from the main model — it can become unavailable independently. When the classifier fails, Bash is fully blocked; read-only tools (Read, Grep, Glob) continue to work. The session then terminated with a 500 Internal Server Error, producing no JSON output. [@session-sdk-decisive-bash-failure] These are transient, recoverable failures — retry is the appropriate response — but they are invisible to `eqgen.ts`'s error handling, which only catches process-level failures from `claude -p`, not SDK-level transport errors.

**Large preset-list output.** As the corpus grows, `tonedeck list --json` output can exceed the SDK's inline tool-result size limit. In session `a91bfb61` the output was 35.4KB; the SDK persisted it to disk and gave the agent a 2KB preview plus a file path. The agent then issued a second `Bash` call grepping the persisted file for the target song name. This two-step pattern works but adds a round-trip. In `claude -p` mode this is not an issue because the list command is never called. [@session-sdk-decisive-hell-of-a-life]
